    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:           
                ...
                messages.error(request, "A master-rating with this name already exists. What about another name?")
                return render(request, "list/master_add.html",
                              {"form": form})
            else:
                messages.success(request, "A MasterRanking was successfully created.")
    ...
    if request.method == "POST":        
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.error(request, "A list with this name already exists. What about another name?")
                form = ListForm(request.POST)
                ...
                l.save()
                messages.success(request, "A list was successfully created")
...
def list_edit(request, list_id, slug):
    try:
        ...
        messages.error(request, "The list doesn't exist")
    ...
    if request.method == "POST":
        ...
        if list_form.is_valid():
            ...
            messages.success(request, "The list was edited")        
...
def list_delete(request, list_id):
    ...
    messages.success(request, "The list was deleted")
    return HttpResponseRedirect(reverse("lists"))
...
def master_delete(request, master_id):
    try:
        ...
        messages.error(request, "The MasterRanking doesn't exist")
        return HttpResponseRedirect(reverse("masters"))
    if master.user != request.user:
        raise Http404
    master.delete()
    ...
    Book.objects.filter(book_in_list__isnull=True).delete()
    Author.objects.filter(author_in_list__isnull=True).delete()
    messages.success(request, "The MasterRanking was deleted")
    return HttpResponseRedirect(reverse("masters"))
...
def bil_edit(request, bil_id): 
    try:
        ...
        messages.error(request, "The book doesn't exist in this list")
        return HttpResponseRedirect(reverse("lists"))
    ...
    if form.is_valid():
        ...
        if bil.list.places_matter and (not form.cleaned_data["place"]):
            messages.error(request, "The book's place in the list should be more than 0")    
            return HttpResponseRedirect(reverse("list", args=(bil.list.id, bil.list.slug))) 
        bil.save()
        ...
        if update_authors: 
            ...
            for master in Master.objects.filter(author_in_master__author__in=affected_authors_set):
                reset_bim_aim_rating(master.id)
        messages.success(request, "The book was edited")
        return HttpResponseRedirect(reverse("list", args=(bil.list.id, bil.list.slug))) 
    messages.error(request, "The book needs a title, and if it has a place, it must be more than 0")    
    return HttpResponseRedirect(reverse("list", args=(bil.list.id, bil.list.slug)))                          
...
def bil_delete(request, bil_id):
    try:
        ...
        messages.error(request, "The book doesn't exist")
        return HttpResponseRedirect(reverse("lists"))
    if book.user != request.user:
        raise Http404
    ...
    messages.success(request, "The book was deleted from the list")
        ...
        if formset.is_valid():
            for form in formset:
                ...
                if form.is_valid():
                    if form.has_changed():
                        ...
                        if message == "List doesn't exist":
                            messages.error(request, message)
                            return HttpResponseRedirect(reverse("lists"))
                        elif message == "If over 100 books on the list have the same title, you can modify the title to avoid confusion":
                            messages.error(request, message)    
            ...
            if list.masters.count() != 0:
                for master in list.masters.all():
                    reset_bim_aim_rating(master.id)
            messages.success(request, "Books were added to the list")
...
def book_add(form, list_id, user):
    ...
    while Book_In_List.objects.filter(book=b, list=list, book__user=user).exists():
        ...
        if i == 100:
            b.delete()
            message = "If over 100 books on the list have the same title, you can modify the title to avoid confusion"
            return message
...
def master(request, master_id, slug):    
    try:
        ...
        messages.error(request, "The master doesn't exist")
        return HttpResponseRedirect(reverse("masters"))
    if master.user != request.user:
        raise Http404
...
def list_details(request, list_id, slug, master_id=""):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, "The book was added to the list")
    if master_id:
            master = Master.objects.get(pk=int(master_id))
